...
/// The waveform rises linearly from -amplitude to +amplitude
pub fn gen(
    comptime T: type,
    options: GenOptions,
) Wave(T) {
...
fn generate_samples(
    ...
    while (i < result.len) : (i += 1) {
        ...
        result[i] = (2.0 * phase - 1.0) * options.amplitude;
    }
...
test "gen" {
    ...
    defer sawtooth.deinit();

    try std.testing.expectEqual(test_data.Sine.len, sawtooth.samples.len);
    for (test_data.Sawtooth, sawtooth.samples) |expected, actual|
